...
## Agent safety: non-destructive deletes

A core design principle is that **agents must never be able to permanently destroy backlog data**. `backlog_delete` (the MCP tool) does not hard-delete items — it moves them to an internal soft-deleted state, invisible to agents but recoverable by humans via the UI or REST API. Agents have no visibility into this mechanism; from their perspective the item is simply gone. Hard delete is only available through the human-facing UI and REST API.

This protects against both accidental and adversarial data loss by agents.
