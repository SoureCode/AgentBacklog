...
Installation, configuration, hooks, slash commands, and MCP setup for **Claude Code**.
...
The plugin registers a `SessionStart` hook (defined in `hooks/hooks.json`) that auto-installs npm dependencies before the MCP server starts:
    ...
    "SessionStart": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "cd ${CLAUDE_PLUGIN_ROOT}/mcp && [ -d node_modules ] || npm install --no-audit --no-fund"
          }
        ]
      }
    ]
...
The MCP server is declared in `.mcp.json` (relative to the plugin root):
...
Claude Code resolves `${CLAUDE_PLUGIN_ROOT}` to the plugin's installation directory.
...
Four specialist agents are bundled and available in Claude Code's agent runner:
